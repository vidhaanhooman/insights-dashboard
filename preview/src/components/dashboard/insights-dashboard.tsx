...
import { ChevronDown, LayoutDashboard, Plus, RefreshCw } from "lucide-react"
...
import { HERO_WIDGET_IDS, SYSTEM_METRICS } from "@/lib/insights/registry"
...
export function InsightsDashboard() {
  ...
  const heroWidgets = widgets.filter((w) => HERO_WIDGET_IDS.includes(w.id))
  const secondaryWidgets = widgets.filter(
    (w) => w.owner === "system" && !HERO_WIDGET_IDS.includes(w.id)
  )
  const customWidgets = widgets.filter((w) => w.owner !== "system")
              ...
              <>
                {heroWidgets.length > 0 && (
                  <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
                    {heroWidgets.map((w) => cell(w, true))}
                  </div>
                )}

                {secondaryWidgets.length > 0 && (
                  <div className="grid grid-cols-1 gap-3.5 md:grid-cols-4">
                    {secondaryWidgets.map((w) => cell(w))}
                  </div>
                )}

                {customWidgets.length > 0 && (
                  <Collapsible defaultOpen>
                    <CollapsibleTrigger className="group flex items-center gap-1.5 text-sm font-medium text-text-muted hover:text-text">
                      <ChevronDown className="size-4 transition-transform group-data-[state=closed]:-rotate-90" />
                      Custom metrics ({customWidgets.length})
                    </CollapsibleTrigger>
                    <CollapsibleContent className="pt-3.5">
                      <div className="grid grid-cols-1 gap-3.5 md:grid-cols-4">
                        {customWidgets.map((w) => cell(w))}
                      </div>
                    </CollapsibleContent>
                  </Collapsible>
                )}
